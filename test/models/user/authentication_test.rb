...
class User::AuthenticationTest < ActiveSupport::TestCase
  ...
  test "creating a session with a non existent email returns nil" do
    @app_session = User.create_app_session(
  ...
  test "can authenticate with valid session id and token" do
    ...
    assert_equal @app_session, @user.authenticate_app_session(
      app_session_id: @app_session.id,
      app_session_token: @app_session.token
    )
  end

  test "trying to authenticate with a token that doesn't exists returns false" do
    @user = users(:jerry)
    assert_not @user.authenticate_app_session(
      app_session_id: 50,
